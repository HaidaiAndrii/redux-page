...
function* onMoviesRequest(payload) {
  ...
  if (cinema[localFileName][title].includes(`${value}`)) {
    ...
    newArr.length > 0
      ? localStorage.setItem(
          ...
          JSON.stringify({
            ...cinema,
            [localFileName]: { ...cinema[localFileName], [title]: [...newArr] },
          })
        )
      : localStorage.setItem(
          "cinema",
          JSON.stringify({ ...cinema, [localFileName]: { [title]: [] } })
        );
  } else {
    let frr = { ...cinema };
  ...
  yield put(
    moviesAC(JSON.parse(localStorage.getItem("cinema"))[localFileName][title])
  );
}
